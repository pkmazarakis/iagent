...
public struct InlineSyntax: Sendable, Equatable {
    ...
    /// Abort when content contains the first character of `close` without the
    /// complete delimiter. Strict punctuation spans use this; HTML-like spans
    /// can disable it so ordinary comparison characters remain valid content.
    public var abortsOnMismatchedCloserPrefix: Bool
    ...
    public init(
        ...
        abortsOnMismatchedCloserPrefix: Bool = true
    ) {
        self.open = open
        self.close = close
        ...
        self.abortsOnMismatchedCloserPrefix = abortsOnMismatchedCloserPrefix
    }
}
            ...
            .map { ext in
                ...
                if let s = ext.inline {
                    parts += ["i", framed(s.open), framed(s.close),
                              ...
                              "\(s.abortsOnMismatchedCloserPrefix)"]
                }
                if let b = ext.block {

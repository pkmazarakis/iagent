...
public struct NativeTextViewWrapper: NSViewRepresentable {
    ...
    public func updateNSView(_ nsView: NSScrollView, context: Context) {
        ...
        // Marker visibility is a presentation setting and may change without
        // rebuilding the SwiftUI wrapper. Sync it live and restyle so switching
        // between source-revealing and strict visual modes takes effect now.
        if context.coordinator.configuration.markers != configuration.markers {
            context.coordinator.configuration.markers = configuration.markers
            textView.configuration.markers = configuration.markers
            let fullRange = NSRange(location: 0, length: (textView.string as NSString).length)
            if fullRange.length > 0, !configuration.rawSourceMode {

...
extension NativeTextViewCoordinator {
    ...
    public func textViewDidChangeSelection(_ notification: Notification) {
        ...
        if let pendingLocation = pendingInlineFormatLocation,
           selRange.length != 0 || selRange.location != pendingLocation {
    ...
    public func textView(_ textView: NSTextView, shouldChangeTextIn affectedCharRange: NSRange, replacementString: String?) -> Bool {
        if !isProgrammaticEdit,
           ...
           affectedCharRange.location == pendingInlineFormatLocation,
           !pendingInlineFormats.isEmpty,
           let replacementString,
           !replacementString.isEmpty,
           replacementString.rangeOfCharacter(from: .newlines) == nil {
            let formats = pendingInlineFormats
            pendingInlineFormats.removeAll()
            pendingInlineFormatLocation = nil

            var wrapped = replacementString
            for format in formats.reversed() {
                wrapped = format.open + wrapped + format.close
            }
            let openingLength = formats.reduce(0) {
                $0 + ($1.open as NSString).length
            }
            ...
            guard textView.shouldChangeText(
                ...
                replacementString: wrapped
            ) else {
                return false
            }
            textView.replaceCharacters(in: affectedCharRange, with: wrapped)

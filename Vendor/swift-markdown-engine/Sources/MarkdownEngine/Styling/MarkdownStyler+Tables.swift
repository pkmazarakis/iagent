...
extension MarkdownStyler {
    ...
    static func styleTables(_ ctx: StylingContext) -> [StyledRange] {
        ...
        let revealsActiveMarkers = ctx.configuration.markers.revealsActiveMarkers
        var neededLengths: Set<Int> = []
        for (idx, token) in tableIndexed
        where !(revealsActiveMarkers && ctx.activeTokenIndices.contains(idx))
        ...
        for (idx, token) in tableIndexed {
            tableCount += 1
            if !(revealsActiveMarkers && ctx.activeTokenIndices.contains(idx)),
            ...
            let isActive = revealsActiveMarkers && ctx.activeTokenIndices.contains(idx)
            if isActive {
